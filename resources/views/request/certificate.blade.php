                    ...
                    <div class="col-sm-12">
                        <div class="card">
                            <div class="card-header">
                                <h5>คำขอหนังสือรับรองกรณีผู้ประสบภัย หรือเจ้าของ หรือผู้ครอบครองทรัพย์สิน
                                    ร้องขอหลักฐานเพื่อรับการสงเคราะห์ หรือบริการอื่นใด</h5>
                            ...
                            <div class="dt-responsive table-responsive">
                                <div class="card-block">
                                    <a href="/request/certificate_insert">
                                    ...
                                    <div class="dt-responsive table-responsive">
                                        <table id="order-table"
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>{{$i}}</td>
                                                    <td class="text-left">{{$item->sufferer_confirm}}</td>
                                                    <td class="text-left"></td>
                                                    <td>{{ date('d-m-Y',strtotime($item->sufferer_date)) }}</td>
                                                    <td>
                                                        <a href="{{url('PDF/certificate',$item->sufferer_id)}}"
                                                        class="btn btn-warning">
                                                        <span class="fa fa-print"></span> พิมพ์เอกสาร</a>
                                                    </td>
                                                    <td>
                                                        <a href="/request/certificate_edit/{{$item->sufferer_id}}"
                                                        class="btn btn-primary m-b-20"><span
                                                        class="fa fa-edit"></span> แก้ไข</a>
                                                    </td>
                                                    <td>
                                                        <form action="/destroy_certificate/{{$item->sufferer_id}}"
                                                            method="post">
                                                            @csrf
                                                            <button type="submit" class="btn btn-danger deleteForm"
                                                                data-name="{{ $item->sufferer_confirm }}">
